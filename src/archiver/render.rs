    ...
    let items: Vec<ListItem> = plugin
        ...
        .map(|ch| {
            ...
            ListItem::new(Line::from(vec![
                ...
                Span::raw(" ".repeat(pad)),
                Span::styled(format!("({platform_label})"), Style::new().fg(Theme::dim())),
            ]))
        })
        .collect();
...
fn render_queue(plugin: &ArchiverPlugin, frame: &mut Frame, area: Rect) {
    if plugin.jobs.is_empty() {
        let lines = vec![
            Line::raw(""),
            Line::styled("  No archive jobs", Style::new().fg(Theme::muted())),
            Line::raw(""),
            Line::styled(
...
pub fn render_recording_picker(
    ...
    if finished.is_empty() {
        let lines = vec![
            ...
            Line::raw(""),
            Line::styled(
                "  Record a stream first, then select recordings to archive.",
                Style::new().fg(Theme::dim()),
            ),
        ];
    ...
    let hint = if sel_count > 0 {
        ...
        " [Enter] Archive  [Space] Select  [f] Filter  [a] Select all  [Tab] Views  [Esc] Back"
            .to_string()
    };
    frame.render_widget(
...
pub fn render_config_modal(plugin: &ArchiverPlugin, frame: &mut Frame, area: Rect) {
    ...
    lines.push(add_field(
        "Archive Dir",
        &draft.archive_dir.display().to_string(),
        false,
        field_idx,
        selected_field,
        editing,
    ...
    lines.push(add_field(
        "Fragments",
        &draft.concurrent_fragments.to_string(),
        false,
        field_idx,
        selected_field,
        editing,
